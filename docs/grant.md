...
- **Ships:** a BR-configured Anchor Platform deployment + two mainnet-bound Soroban primitives (SEP-38 rate-lock, partner attribution; audited via the SDF Soroban Audit Bank before mainnet) + a card-collateral smart-account testnet PoC + public TS + Python SDKs. All three contracts compose OpenZeppelin's audited `stellar-contracts =0.7.1` building blocks, keeping Bleu's novel surface small and the audit scope tight.
