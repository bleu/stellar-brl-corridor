...
| **OpenZeppelin Stellar Contracts `=0.7.1`** (MIT, audited by OZ, SDF collaboration) | Audited building blocks (composed today) | **Composed now**, not aspirational. Wired into all three contracts: `stellar_fee_abstraction::validate_expiration_ledger` (rate-lock expiry in fx-rate-lock); `stellar_tokens::fungible::sac_admin_wrapper` over USDC's deterministic SAC + `stellar_access::access_control` (partner-attribution); `stellar_contract_utils::pausable` + `stellar_access::access_control` (card-collateral PoC). `SmartAccount` + `policies::spending_limit` + `verifiers::webauthn`/`ed25519` remain the production-vault target for card-collateral. **Version note:** OZ 0.7.1 (latest release) requires `soroban-sdk ^25.3.0`; the workspace is pinned to `=25.3.0` to compose `=0.7.1` for real — no OZ release targets `soroban-sdk 26` yet. **Out of OZ scope (always Bleu-specific)**: `partner_transfer` event, SEP-38 quote-hash lifecycle, shortfall invariant. |
